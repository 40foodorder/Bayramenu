...
class OrderAdapter(private val onAction: (Order, OrderStatus) -> Unit) : RecyclerView.Adapter<OrderAdapter.OrderViewHolder>() {
    ...
    override fun onBindViewHolder(holder: OrderViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            val nextStatus = when(order.status) {
                ...
                OrderStatus.PREPARING -> OrderStatus.DELIVERED // Ready for Pickup simulation
                else -> order.status
            }
    ...
    class OrderViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        fun bind(order: Order) {
            ...
            itemView.findViewById<TextView>(android.R.id.text2).text = "Tap to move to next stage"
        }
    }
}
